...
public static String[] life(String[] c)
{
		...
		for(int i= 0;i < cells[row].length();i++)
	{
	   ...
	   if(i == 0)
	  {
	    above= (row == 0) ? null : cells[row - 1].substring(i, i + 2);
	    same= cells[row].substring(i + 1, i + 2);
	    below= (row == cells.length - 1) ? null : cells[row + 1].substring(i, i + 2);
	      }
	      else if(i == cells[row].length() - 1)
	      {
		above= (row == 0) ? null : cells[row - 1].substring(i - 1, i + 1);
		same= cells[row].substring(i - 1, i);
		below= (row == cells.length - 1) ? null : cells[row + 1].substring(i - 1, i + 1);
      }
      else
	{
		above= (row == 0) ? null : cells[row - 1].substring(i - 1, i + 2);
	      	same= cells[row].substring(i - 1, i) + cells[row].substring(i + 1, i + 2);
		below= (row == cells.length - 1) ? null : cells[row + 1].substring(i - 1, i + 2);
      }
